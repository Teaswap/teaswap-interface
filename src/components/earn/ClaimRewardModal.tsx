...
import { IdoInfo, StakingInfo } from '../../state/stake/hooks'
import { useIdoContract, useStakingContract } from '../../hooks/useContract'
...
export default function ClaimIdoRewardModal({ isOpen, onDismiss, idoInfo }: IdoModalProps) {
  const { account } = useActiveWeb3React()
  const { t } = useTranslation()
  ...
  async function onClaimReward() {
    if (idoContract && idoInfo?.makeAmount) {
      setAttempting(true)
      await idoContract
        .getReward({ gasLimit: 350000 })
        .then((response: TransactionResponse) => {
      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          ...
          {idoInfo?.earnedAmount && (
            <AutoColumn justify="center" gap="md">
              <TYPE.body fontWeight={600} fontSize={36}>
                {idoInfo?.earnedAmount?.subtract(idoInfo.claimedAmount).toSignificant(6)}
              </TYPE.body>
              <TYPE.body>{t('unclaimed')} {idoInfo?.tokens[1].symbol}</TYPE.body>
            </AutoColumn>
          )}
          <TYPE.subHeader style={{ textAlign: 'center' }}>
