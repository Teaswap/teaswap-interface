...
import {BETH, BUSD, CJAI, ICASH, NFTEXCHANGE, PAYABLEETH, SHIH, TSALOT, UNI, ZERO_ADDRESS, XCOIN} from "../../constants";
...
export const ProductInfo = ({product,user}:{ product:ProductInterface, user:userMiniInterface}) => {
  ...
  const tokenOptions = [
    ...
    { name: 'ICASH',address:ICASH.address,token:ICASH},
    { name: 'Xcoin',address: XCOIN.address,token:XCOIN}
  ]
  const {
    errorMessage,
