...
import { Currency, ETHER, Token, currencyEquals } from '@teaswap/uniswap-sdk'
...
export function useToken(tokenAddress?: string,isNFT?:boolean): Token | undefined | null {
  ...
  const tokenName = useSingleCallResult(token ? undefined : tokenContract, 'name', undefined, isNFT?undefined:NEVER_RELOAD)
  ...
  const symbol = useSingleCallResult(token ? undefined : tokenContract, 'symbol', undefined, isNFT?undefined:NEVER_RELOAD)
