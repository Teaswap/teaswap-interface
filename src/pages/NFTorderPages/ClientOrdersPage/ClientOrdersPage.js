...
const Content = styled.td`
  font-size: ${FONT.xsm};
  color: ${COLOR.text_2};
  border-bottom: solid 0.5px ${COLOR.cccccc};
  padding: ${DISTANCE.sm};
  cursor: pointer;
...
const ClientOrdersPage = () => {
  ...
  const { carts, handleGetCart } = useCart();

  const { orders, handleGetClientOrder, handleGetSellerOrder } = useOrder();
                      ...
                      order.Order_items.map((item) => (
                        <ContentContainer key={order.id}>
                          <OrderImg
                            onClick={() =>
                              navigate(
                                `/nft/products/${order.Order_items[0].ProductId}`
                              )
                            }
                          ...
                          <Content>
                            {item.product_price +
                              handleTokenSwitch(item.extoken)}
                          </Content>
                          <Content>
